...
function buildSelfAssessment(input) {
  ...
  if (!hasAnyJudgeable) {
    ...
    answerJa = `YES — ${posParts.join("、")}。統計的に意味のある悪化はありません`
      // v88(2026年9月9日・本番初日の実測で発見した文言バグの修正): 旧文言
      // 「本日の的中率は下がっていますが…」は方向を決め打ちしており、的中率が
      // **上がった**日(実測: 前日30%→本日63.6%、+33.6pt)にも「下がっています」と
      // 表示していた。方向に依存しない正確な文へ修正(判定ロジック自体は不変)。
      + (referenceAxes.some((a) => /揺れ|下回|低下/.test(a.valueJa)) ? "(本日の的中率の上下は、答え合わせ件数が少なく統計的な揺れの範囲のため判定には使っていません)" : "")
      + "。";
  } else {
