...
const LEARN_ERROR_RULES = [
  ...
  // ---- v55(第11次監査)で追加: 選手収集の個別失敗 ----
  //   本番で「Guillem Badiaの統計取得失敗」1件が受け皿(影響あり)に落ち、
  //   サイト影響級の警告として表示されていた。実際の影響は「その選手の
  //   データだけ古いまま」で、v44の自己回復(3回失敗で30日スキップ+巡回は
  //   前進)が既に働いている。実態に合う説明で minor に分類する。
  {
    id: "universePlayer",
    test: /^universe_player_failed/,
    level: "minor",
    titleJa: "データを取得できなかった選手がいます",
    meaningJa: "選手索引の巡回中に、特定の選手の統計取得が提供元側で失敗しました(移籍直後や提供元のデータ不備で起こります)。",
    effectJa: "その選手のデータだけが更新されません。巡回は次の選手へ進み、同じ選手で3回失敗すると30日間自動でスキップします(自己回復済みの設計)。",
  },
